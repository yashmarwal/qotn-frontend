...
export default function CartDrawer() {
  ...
  return (
    <>
      {isOpen && (
        <div onClick={closeCart} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(26,26,26,0.3)', zIndex: 80 }} />
      )}
      <div
        style={{
          position: 'fixed', top: 0, right: 0, height: '100%', width: '100%', maxWidth: 440,
          backgroundColor: 'var(--cream)', zIndex: 90,
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 28px', borderBottom: '1px solid var(--border)' }}>
          <span style={{ fontSize: 13, letterSpacing: '0.12em', fontWeight: 500, textTransform: 'uppercase' }} suppressHydrationWarning>
            Bag ({totalItems})
          </span>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 28px' }}>
          {items.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60%', gap: 16 }}>
              ...
              <button onClick={closeCart} style={{ fontSize: 12, letterSpacing: '0.10em', textTransform: 'uppercase', background: 'none', border: 'none', color: 'var(--black)', textDecoration: 'underline', cursor: 'pointer' }}>
                Continue Shopping
              </button>
            </div>
          ) : (
            <div>
              {items.map((item, idx) => (
                <div
                  ...
                  style={{ display: 'flex', gap: 16, padding: '20px 0', borderBottom: idx < items.length - 1 ? '1px solid var(--border)' : 'none' }}
                >
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 14 }}>{item.product.name}</span>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--dust)' }}>{item.customStitchingId ? 'Custom Size' : item.size} · {item.color}</span>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                      <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border)' }}>
                        ...
                        <span style={{ fontSize: 13, padding: '0 8px', minWidth: 24, textAlign: 'center' }}>{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.product.id, item.size, item.quantity + 1)} style={{ background: 'none', border: 'none', padding: '6px 10px', cursor: 'pointer' }}>
                          <Plus size={12} strokeWidth={1.5} />
                        </button>
                      </div>
                      <span style={{ fontSize: 14, fontWeight: 500 }}>{formatPrice(item.product.price * item.quantity)}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {items.length > 0 && (
          <div style={{ padding: '20px 28px', borderTop: '1px solid var(--border)' }}>
            ...
            <button onClick={closeCart} style={{ width: '100%', marginTop: 12, background: 'none', border: 'none', fontSize: 12, color: 'var(--dust)', cursor: 'pointer', textDecoration: 'underline' }}>
              Continue Shopping
            </button>
          </div>
        )}
