...
export default function Footer() {
  return (
    <footer style={{ backgroundColor: 'var(--black)', color: 'var(--cream)' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '80px 40px 48px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 48 }}
        className="footer-grid">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <span className="brand-wordmark" style={{ fontSize: 20 }}>QOTN</span>
          <p style={{ fontSize: 12, letterSpacing: '0.12em', color: 'rgba(245,240,232,0.5)', textTransform: 'uppercase' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ marginTop: 24 }}>
            <p style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(245,240,232,0.5)', marginBottom: 12 }}>Newsletter</p>
            <div style={{ display: 'flex', gap: 0 }}>
              ...
              <button style={{ padding: '10px 16px', background: 'var(--cream)', color: 'var(--black)', border: 'none', fontSize: 11, letterSpacing: '0.08em', fontWeight: 500, cursor: 'pointer' }}>
                JOIN
              </button>
            </div>
          </div>
      ...
      <style jsx>{`
        @media (max-width: 768px) {
          .footer-grid { grid-template-columns: 1fr 1fr !important; gap: 40px !important; padding: 48px 24px 40px !important; }
        }
